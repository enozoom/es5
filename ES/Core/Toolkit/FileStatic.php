...
final class FileStatic {
     ...
     */
    public static function mkdir(string $filepath,$timedir=FALSE):string
    {
     ...
     * @param strint $file
     */
    public static function write(string $str,string $file):int{
            return file_put_contents($file,$str,FILE_APPEND);
    }
    
    /**
     * 递归扫描文件夹，按条件获取文件夹路径
     * @param string $files  所有扫描到的文件
     * @param string $root 要所描的文件夹
    ...
    public static function scanDir(&$files,$root = './',$funcFilter=''){
            ...
            foreach (scandir($root) as $d){
                    ...
                    if( is_dir($_d) ){// 文件夹
                        in_array(basename($_d),['.','..']) || self::scanDir($files, $_d, $funcFilter);
                    }else{
                        $funcFilter($d) && $files[$d] = $_d;
                    }
            }
    }
     ...
     */
    public static function delDir(string $dirPath){
            if(is_dir($dirPath)){
     ...
     * @param string $target
     */
    public static function mergeDir(string $source,string $target){
        $path = function($_path){
